...
export function Home() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-16 md:py-24">
      ...
      <div className="relative group max-w-2xl mb-10 sm:mb-16 md:mb-24">
        ...
        <div className="relative">
          <div className="absolute inset-y-0 left-0 pl-4 sm:pl-5 flex items-center pointer-events-none">
            <Search className="h-5 w-5 text-zinc-500 group-focus-within:text-violet-400 transition-colors shrink-0" />
          </div>
          <input
            type="text"
            ...
            className="w-full bg-zinc-900/80 border border-zinc-800 rounded-xl sm:rounded-2xl py-3.5 sm:py-4 pl-12 pr-4 text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition-all shadow-sm text-base min-h-[48px]"
          />
        </div>
      </div>
      ...
      <div className="mb-10 sm:mb-16 md:mb-20 relative">
        <div className="absolute -inset-x-4 sm:-inset-x-6 -inset-y-6 sm:-inset-y-8 bg-gradient-to-b from-zinc-900/50 to-transparent rounded-3xl sm:rounded-[3rem] -z-10 pointer-events-none"></div>

        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          <div className="h-6 sm:h-8 w-1 sm:w-1.5 bg-fuchsia-500 rounded-full shrink-0"></div>
          <h2 className="text-xl sm:text-2xl font-bold text-zinc-50 tracking-tight">Continue Estudando</h2>
        </div>

        {!hasProgress ? (
          <div className="relative overflow-hidden bg-gradient-to-r from-fuchsia-500/10 to-violet-500/10 border border-fuchsia-500/20 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-6 sm:gap-8 group hover:border-fuchsia-500/40 transition-colors">
            ...
            <div className="relative z-10">
              <h3 className="text-xl sm:text-2xl font-bold text-zinc-50 mb-2 sm:mb-3 tracking-tight">Pronto para começar?</h3>
              <p className="text-zinc-400 text-base sm:text-lg">Você ainda não começou a estudar. Comece pelo primeiro módulo.</p>
            </div>
            {firstModule && firstTopic && (
              <Link
                to={`/module/${firstModule.id}/topic/${firstTopic.id}`}
                className="relative z-10 inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-semibold transition-all hover:shadow-lg hover:shadow-fuchsia-500/25 hover:-translate-y-0.5 whitespace-nowrap min-h-[48px] active:scale-[0.98]"
              >
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-6 sm:gap-8 group hover:border-emerald-500/40 transition-colors">
            ...
            <div className="relative z-10">
              <h3 className="text-xl sm:text-2xl font-bold text-zinc-50 mb-2 sm:mb-3 tracking-tight">Parabéns! 🎉</h3>
              <p className="text-zinc-400 text-base sm:text-lg">Você concluiu todos os tópicos disponíveis.</p>
            </div>
            <div className="w-full sm:w-56 hidden sm:block relative z-10">
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-fuchsia-500/10 to-violet-500/10 border border-fuchsia-500/20 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl flex flex-col md:flex-row items-stretch md:items-center justify-between gap-6 sm:gap-8 group hover:border-fuchsia-500/40 transition-colors">
            ...
            <div className="flex-1 min-w-0 relative z-10">
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
                ...
                <span className="text-zinc-400 text-xs sm:text-sm font-medium flex items-center gap-2">
              ...
              <h3 className="text-2xl sm:text-3xl font-bold text-zinc-50 mb-2 sm:mb-3 tracking-tight leading-tight">{nextTopic.title}</h3>
              <p className="text-zinc-400 text-base sm:text-lg line-clamp-2 mb-4 md:mb-0 max-w-2xl">{nextTopic.description}</p>
            ...
            <div className="flex flex-col items-stretch md:items-end gap-4 sm:gap-6 w-full md:w-auto shrink-0 relative z-10">
              <div className="hidden md:block w-56">
                <ProgressBar value={Math.round((nextModule.topics.filter(t => isCompleted(t.id)).length / nextModule.topics.length) * 100)} />
              </div>
              <Link
                to={`/module/${nextModule.id}/topic/${nextTopic.id}`}
                className="w-full md:w-auto inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-semibold transition-all hover:shadow-lg hover:shadow-fuchsia-500/25 hover:-translate-y-0.5 min-h-[48px] active:scale-[0.98]"
              >
                Continuar
      ...
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        <div className="h-6 sm:h-8 w-1 sm:w-1.5 bg-zinc-700 rounded-full shrink-0"></div>
        <h2 className="text-xl sm:text-2xl font-bold text-zinc-50 tracking-tight">Módulos</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
        {filteredModules.map((module) => {
          ...
          return (
            <div key={module.id} className="flex flex-col gap-3 sm:gap-4">
              <div className="group/card relative h-full">
                <div className="absolute -inset-0.5 bg-gradient-to-b from-zinc-800 to-zinc-900 rounded-2xl sm:rounded-[1.75rem] opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
              ...
              {module.matchedTopics && module.matchedTopics.length > 0 && (
                <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-xl sm:rounded-2xl p-4 sm:p-5 flex flex-col gap-3 shadow-sm mt-1 sm:mt-2 relative z-10">
                  <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider flex items-center gap-2">
                    <BookOpen className="w-3.5 h-3.5 shrink-0" />
                    Tópicos encontrados
                  </h4>
                  <div className="space-y-2 mt-0 sm:mt-1">
                    {module.matchedTopics.map(topic => (
                      <div key={topic.id} className="group/topic flex items-center justify-between gap-3 sm:gap-4 p-3 sm:p-3 rounded-xl hover:bg-zinc-800/50 transition-colors border border-transparent hover:border-zinc-700/50">
                        <span className="text-sm text-zinc-300 font-medium truncate min-w-0 group-hover/topic:text-zinc-100 transition-colors">{topic.title}</span>
                        <Link
                          to={`/module/${module.id}/topic/${topic.id}`}
                          className="shrink-0 px-4 py-2.5 rounded-lg bg-zinc-800 text-zinc-300 hover:bg-violet-600 hover:text-white text-xs font-medium transition-all shadow-sm min-h-[44px] flex items-center justify-center active:scale-[0.98]"
                        >
                          Abrir
                        </Link>
                      </div>
      ...
      <div className="mt-12 sm:mt-16 md:mt-24 bg-zinc-900/60 border border-zinc-800/80 rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-bold text-zinc-50 mb-0.5 sm:mb-1 tracking-tight">Progresso da Jornada</h2>
            <p className="text-zinc-400 text-xs sm:text-sm">
              {completedTopicsCount} de {totalTopics} tópicos concluídos
            </p>
          </div>
          <div className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400 tabular-nums">
            {globalProgressPercentage}% <span className="text-xs sm:text-sm font-medium text-zinc-500 tracking-normal">da jornada completa</span>
          </div>
        </div>
